...
sym_indef = ghs_indef.loc[conditions].merge(
    ss_stats, on="id", suffixes=(None, "_stats")
)
...
if not FORCE_UPDATE and pkl_file.exists():
    ...
    df = pd.DataFrame(index=sym_indef["id"], columns=SEEDS.keys(), dtype=float)
...
# bins = np.logspace(
#     np.log10(tf["time ratio"].min()), np.log10(tf["time ratio"].max()), 20
# )
...
sns.kdeplot(tf, x="time ratio", hue="seed", log_scale=True, common_grid=True, ax=ax)
